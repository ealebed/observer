...
- Required check: `ci / validate / Validate golang layer` (observed on PRs from [Container image CI](../.github/workflows/wfl_image_ci.yaml)). Do **not** require `ci / build-push-image`; that job is skipped on pull requests.
- Container image CI path filters include `go.mod` / `go.sum`, `Dockerfile`, `internal/**.go`, `cmd/**.go`, and `.github/workflows/**`, so gomod, docker, and github-actions Dependabot PRs all report the required check.
