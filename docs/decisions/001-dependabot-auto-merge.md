...
- This repository is personal (`ealebed/observer`), so organization ruleset bypass lists are not available.
...
- [Container image CI](../../.github/workflows/wfl_image_ci.yaml) uses path filters. If those filters skip the workflow, the required check never reports and auto-merge cannot complete.
...
Widen Container image CI path filters to `.github/workflows/**` so GitHub Actions Dependabot PRs (including this auto-merge workflow) still run golang validate.
...
### Leave CI path filters as `wfl_image_ci.yaml` + `tpl_*.yml` only
- Pros: Fewer golang validate runs for workflow-only PRs.
- Cons: Actions bumps in other workflow files never report the required check, so auto-merge waits forever.
- Rejected: The required check must run for every Dependabot ecosystem this repo tracks.
